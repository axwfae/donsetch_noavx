# donsetch_noavx_250 — 修改說明（給下次修改的人/AI 看）

> **閱讀時機**：任何要修改本目錄、或要把 noavx 修改移植到新版 donsetch 時，先讀完這份文件再動手。

## 1. 這個目錄是什麼

`donsetch_noavx_250/` 是 **donsetch-2.5.0 的 noavx 建置變體**，仿照 `donsetch_noavx/`（2.3.4 版）的結構精簡而來。
...
**目錄定位**：本目錄只攜帶 noavx 建置設定、CI workflow、文件，以及 **`src/` 中「有修改」的檔案**（目前僅 `src/cli/{update,version,status}.rs` 三個，見第 3 節）。未修改的原始碼一律不同步進來，以免與基礎版產生干擾——建置時把本目錄疊在 `donsetch-2.5.0` 原始碼樹上使用。升級版本時需重新套用這些檔案修改（見第 5 節）。

## 2. 各檔案修改內容與原因

### Cargo.toml
- `oar-ocr` features 改為 `["simd"]`、`ort` features 改為 `["std", "ndarray", "copy-dylibs"]`
  → **移除預設的 `download-binaries`**：不能再用 pyke.io 的 AVX 二進位檔。
- 新增頂層 features：
...
### .github/workflows/ci.yml（github/workflows/ci.yml 為相同鏡像）
...
### .github/workflows/release.yml（github/workflows/release.yml 為相同鏡像）
...
- **刻意保留 2.5.0 的升級**：`actions/download-artifact@v8`（donsetch_noavx 2.3.4 版還是 v4）。
...
**修改位置：`src/cli/{update.rs,version.rs,status.rs}` 三個檔案的 `REPO` 常數**（本目錄的 `src/` 只同步這三個修改過的檔案；`donsetch-2.5.0/src/cli/` 亦已同步同樣內容）
...
**fork Release 格式需求**：tag 為 `v2.x.x`，資產名稱 `donsetch-linux-x64.tar.gz`（+ `.sha256`）與 `donsetch-linux-x64-noavx.tar.gz`——本目錄的 release workflow 產出的正是這些名稱。

## 4. 對照基準與驗證方式

- 對照組：`donsetch_noavx/`（2.3.4 的 noavx 變體）、`donsetch-2.5.0/`（乾淨的上游版）。
- 驗證指令：
  ```bash
  diff -r donsetch_noavx_250 donsetch_noavx   # 差異應只有：2.5.0 內容漂移、download-artifact@v8、版本號
  python3 -c "import tomllib; tomllib.load(open('donsetch_noavx_250/Cargo.toml','rb'))"
  python3 -c "import yaml; yaml.safe_load(open('donsetch_noavx_250/.github/workflows/ci.yml'))"
  grep -rn "dondai44423" donsetch_noavx_250/   # 應無殘留（README/CONTRIBUTING 已全數改為 axwfae/donsetch_noavx）
  ```
- `.github/workflows/` 與 `github/workflows/` 必須保持完全一致（歷史上游同時維護兩處）。
- 本目錄的 `src/` 只含修改過的檔案（`src/cli/{update,version,status}.rs`），`donsetch_noavx/` 沒有 src，diff -r 時會顯示為「Only in donsetch_noavx_250」，屬預期。可用 `diff donsetch_noavx_250/src/cli/{update,version,status}.rs` 對應 `donsetch-2.5.0/src/cli/` 同名檔驗證完全一致；除此之外本目錄不得出現其他 `src/` 檔案。
...
4. workflows 以本目錄版本為範本，但 port 新版的 action 版本升級與 apt 改進。
